...
class Business(models.Model):
	...
	def get_event_schedule(self):
		...
		for event in self.event_set.all():
			event_dict = {}
			event_dict["id"] = str(event.pk)
			event_dict["title"] = str(event.title)
			event_dict["url"] = "/event/{}".format(event.pk)
			event_dict["class"] = "event-special"
			event_dict["start"] = str(event.date_from.timestamp() * 1000)
			event_dict["end"] = str(event.date_to.timestamp() * 1000)
